...
public class CmdGather {
    ...
    public static Command gather(RobotContainer r){
        ...
        Command c = new WaitCommand(startupTime).deadlineWith(new RunCommand( () -> r.gather.setGatherPower(intakePower, gatePower)));
        //detect when the piece has made it to the gate wheel
        c = c.andThen(new WaitUntilCommand(() -> r.gather.getGateCurrent() > detectGateCurrent)).finallyDo(() -> r.gather.setGatePower(0));
        ...
        c = c.andThen(new WaitCommand(extraIntakeTime)).finallyDo(() -> r.gather.setIntakePower(0));
